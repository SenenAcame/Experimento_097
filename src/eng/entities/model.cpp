...
void Model::Draw(RShader &shader) {
...
std::vector<RTexture> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName) {
    std::vector<RTexture> textures;
    for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {
            if(std::strcmp(textures_loaded[j].texImage_.data(), str.C_Str()) == 0)
            {
        ...
        {   // if texture hasn't been loaded already, load it
            ...
            RTexture texture = {thisTextureFromFile(str.C_Str(), this->directory)};
            texture.type_ = typeName;
            texture.texImage_ = str.C_Str();
...
unsigned int thisTextureFromFile(const char *path, const std::string &directory, bool gamma) {
