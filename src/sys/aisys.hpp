...
struct AISys {
    ...
    static constexpr auto PI { std::numbers::pi };
     ...
     double arcTan(double const y, double const x) const noexcept{
     ...
     double distancePoints(double const a, double const b) const noexcept{
        return a - b;
    }

     double distanceModule(double const x, double const y) const noexcept{
        return std::sqrt(x*x + y*y);
    }

     double distanceAngle(double const a, double const b) const noexcept{
        double angDist = a - b;
     ...
     void arrive(AICmp& a, PhysicsCmp2& p) const noexcept{
        p.v_ang = p.a_lin = 0;
