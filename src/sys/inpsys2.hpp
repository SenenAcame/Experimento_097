    ...
    void update(EntyMan& EM, TheEngine& eng, SoundSystem_t& SS, double const dt) {
        ...
        EM.foreach<SYSCMPs, SYSTAGs>(
            [&](Enty& player, InputCmp2& i, RenderCmp2& r, PhysicsCmp2& p) {
                ...
                if(keyboard.isKeyPressed(i.key_up))    { p.v_lin =  20; }
                if(keyboard.isKeyPressed(i.key_down))  { p.v_lin = -20; }
                if(keyboard.isKeyPressed(i.key_right)) { p.v_lin =  20; p.v_ang =  90; }
                if(keyboard.isKeyPressed(i.key_left))  { p.v_lin = -20; p.v_ang =  90; }
