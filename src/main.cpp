...
int main(){
    ...
    unsigned char *data = stbi_load("assets/shaders/container.jpg", &width, &height, &nrChannels, 0);
    if(data){
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    //load image, create texture and generate mipmaps
    data = stbi_load("assets/shaders/awesomeface.png", &width, &height, &nrChannels, 0);
    if(data){
    ...
    ourShader.use(); //SET SHADER BEFORE SETTING UNIFORMS
    //set it mannually 
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        for(unsigned int i = 0; i<10; i++){
            ...
            float angle = 20.0f * i;
            if(i % 3 == 0) //every 3rd iteration, set the angle
    ...
    if(glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS){
     ...
     if(glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS){
      ...
      if(glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS){
        cameraPos -= (glm::normalize(glm::cross(cameraFront, cameraUp)) * 0.25f);
      }
...
void mouse_callback(GLFWwindow* window, double xposIn, double yposIn){
    ...
    //make sure when pitch is out of bounds, screen doesnt get flipped
    if(pitch > 89.0f){
        pitch = 89.0f;
    }
    if(pitch < -89.0f){
